...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Part of Flaw 2, row 56 (saving user with unhashed password)
            form.save()
            return redirect(reverse('polls:index'))
            # Fix to flaw 2, rows 59-62
